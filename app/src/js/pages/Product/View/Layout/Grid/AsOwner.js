    ...
    <div className='edit-product-container' style={{ background: getBackground(product) }}>
      <Grid celled='internally'>
        ...
        <Grid.Row columns={2}>
          ...
          <Grid.Column width={8} stretched>
            <Segment>
              <Button fluid basic color='red' onClick={() => deleteProduct(product.id, product.profileId, user)} style={{ justifyContent: 'center' }}>Remove listing</Button>
            </Segment>
          </Grid.Column>
        </Grid.Row>
      </Grid>
...
({
  deleteProduct: (productId, profileId, user) => dispatch(deleteProduct(productId, profileId, user)),
